...
```bash
mvn package
java -jar target/helidon-mp-database.jar
```
...
```
docker build -t helidon-mp-database .
```

## Running the Docker Image

```
docker run --rm -p 8080:8080 helidon-mp-database:latest
```
...
kubectl get service  helidon-mp-database                     # Get service info
kubectl port-forward service/helidon-mp-database 8081:8080   # Forward service port to 8081
```
...
The target/helidon-mp-database-jri directory is a self contained custom image of your application. It contains your application,
its runtime dependencies and the JDK modules it depends on. You can start your application using the provide start script:

```
./target/helidon-mp-database-jri/bin/start
```
